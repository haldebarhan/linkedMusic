...
export class ServiceTypeService {
  ...
  private async checkCategory(categoryId: number, categoryParentId?: number) {
    const [category, parent] = await Promise.all([
      this.categoryRepository.findOne(categoryId),
      categoryParentId
        ? this.categoryRepository.findOne(categoryParentId)
        : Promise.resolve(null),
    ]);
    if (!category) throw createError(404, "category not found");
    if (categoryParentId && !parent)
      throw createError(404, "category parent not found");
  }
